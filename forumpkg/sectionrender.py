...
def rendersections(config, db, tmpr, prnt_id=None):
   ...
   if len(data) != 0:
      ...
      for x in data:
         ...
         tmp = tmpr.MkPageFromFile("templates/inner_templates/msg_tmp.xml", {'text' : "<a href = index?action=showmessage&value=" + str(x[0]) + "> " + str(x[1]) + "</a>", 'date' : str(x[2]), 'username' :  udata[0][0] })
         res += tmp 
      res += '</table>'
   else:
      res += '</br>No themes'
